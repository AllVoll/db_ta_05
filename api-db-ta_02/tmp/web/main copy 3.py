...
from .models import ApiKey, Strategy, Bot, Wallet
...
def check_tables_exist():
    for table in [ApiKey.__table__, Strategy.__table__, Bot.__table__, Wallet.__table__]:
        if not engine.dialect.has_table(engine, table.name):
            return False
    return True

# Создаем таблицы при запуске приложения, если они не существуют
